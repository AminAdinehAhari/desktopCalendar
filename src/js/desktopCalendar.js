(function() {
  ...
  this.desktopCalendar = function() {
      ...
      if (arguments[0] && typeof arguments[0] === "object") {
        ...
        this.options.defualtIcon = extendDefaults(this.options.defualtIcon, this.options.icons);
        this.options.defualtLag  = extendDefaults(this.options.defualtLag, this.options.lang);
  ...
  desktopCalendar.prototype.createThemplate = function() {
    ...
    this.options.selecItem.classList.add(getThemeClass(this.options));
    this.options.selecItem.classList.add(getColoringClass(this.options));
    this.options.selecItem.setAttribute("lang-dir", this.options.direction );
    ...
    delegate( this.options.selecItem , "click", ".dc--previwPageYear", function(event) {
      yearFilling(ths.options , parseInt( ths.options.selecItem.querySelectorAll(".dc--yearModalContent>.year-item:last-child")[0].getAttribute("year-index") ) + 8 );
    });

    delegate( this.options.selecItem , "click", ".dc--nextPageYear", function(event) {
      yearFilling(ths.options , parseInt( ths.options.selecItem.querySelectorAll(".dc--yearModalContent>.year-item:first-child")[0].getAttribute("year-index") ) - 8 );
    });
  ...
  function setFillTabel(){
    ...
    selectDefualtTabelCell(this.options);
  ...
  function selectDefualtTabelCell(optn){
  ...
  function setDayEvent(optn , td ) {
    ...
    for (var i = 0; i < holiday.length; i++) {
      eventhml += `<li class="holiday"><b>${ holiday[i].title }</b> (${optn.defualtLag.holiday}) </li>` ;
    }
      ...
      return `
        <div class="dc--yearModal dc--Modal">
          <div class="dc--ModalBody">
            <button class="dc--close">${ optn.defualtIcon.close }</button>
            <div class="dc--ModalContent">
              <div class="dc--yearModalContent">
              </div>
              <button class="dc--nextPageYear">${ optn.defualtIcon.rightArrow }</button>
              <button class="dc--previwPageYear">${ optn.defualtIcon.leftArrow }</button>
            </div>
          </div>
        </div>`;
    }
  ...
  function createBody(optn){
      ...
      <div class="dc--body animate-transition">
        ...
        <div class="dc--mainbar animate-transition">
          <button class="dc--toggleSidebar animate-transition">${ optn.defualtIcon.rightArrow }</button>
          ...
          <div class="dc--calendarFooter">
              <h5 class="calendarEventTitle" >${ optn.defualtLag.events }:</h5>
  ...
  function sidebarToggleIconChange(optn){
    ...
    if( (" " + item.className + " ").replace(/[\n\t]/g, " ").indexOf(" dc--showBar ") == -1  ){
      if (bodyWidth(optn) < getBreakingPoint(optn) ) {
        sel.querySelectorAll(".dc--toggleSidebar")[0].innerHTML  = optn.defualtIcon.menu;
      }else{
        sel.querySelectorAll(".dc--toggleSidebar")[0].innerHTML  = optn.defualtIcon.leftArrow;
      }
    }else{
      if (bodyWidth(optn) < getBreakingPoint(optn) ) {
        sel.querySelectorAll(".dc--toggleSidebar")[0].innerHTML  = optn.defualtIcon.close;
      }else{
        sel.querySelectorAll(".dc--toggleSidebar")[0].innerHTML  = optn.defualtIcon.rightArrow;
      }
    }
  }
